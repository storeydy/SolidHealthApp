...
var supported = (algorithm) => {
    ciphers || (ciphers = new Set(crypto.getCiphers()));
...
let oneShotSign;
if (crypto__namespace.sign.length > 3) {
    oneShotSign = util.promisify(crypto__namespace.sign);
}
else {
    oneShotSign = crypto__namespace.sign;
}
...
let oneShotVerify;
if (crypto__namespace.verify.length > 4 && oneShotCallbackSupported) {
    oneShotVerify = util.promisify(crypto__namespace.verify);
}
else {
    oneShotVerify = crypto__namespace.verify;
}
