...
globals_1.describe("getSessionIdFromOauthState", () => {
    globals_1.it("returns stored OIDC 'state' for request's OIDC 'state' value", async () => {
        ...
        await globals_1.expect(StorageUtility_1.getSessionIdFromOauthState(mockedStorage, oauthState)).resolves.toBe(oauthStateValue);
    });
...
globals_1.describe("saveSessionInfoToStorage", () => {
    ...
    globals_1.it("saves the DPoP key if provided in the given storage", async () => {
        const mockedStorage = StorageUtility_2.mockStorageUtility({});
        const dpopKey = await mockKeyPair();
        await StorageUtility_1.saveSessionInfoToStorage(mockedStorage, "some session", "an ID token", "https://my.webid", "true", "a refresh token", true, dpopKey);
        globals_1.expect(JSON.parse((await mockedStorage.getForUser("some session", "publicKey", {
            secure: true,
        })))).toEqual(dpopKey.publicKey);
        const privateJwk = await mockedStorage.getForUser("some session", "privateKey", { secure: true });
        globals_1.expect(JSON.parse(privateJwk)).toEqual(await jose_legacy_modules_1.fromKeyLike(dpopKey.privateKey));
    });
});
